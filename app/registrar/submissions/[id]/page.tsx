...
type AttachmentWithMeta = AttachmentRecord & {
  signedUrl?: string;
  uploaderName: string;
};
...
export default function SubmissionDetailsPage() {
  ...
  const [fileHistory, setFileHistory] = useState<AttachmentWithMeta[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const attachmentRecords = (attData || []) as AttachmentRecord[];
        ...
        (
          ...
          uploaderMap.set(p.id, `${p.first_name} ${p.last_name} (${p.role})`)
        );
        (
        ...
        setApprovalHistory((appData || []) as ApprovalStep[]);
      } catch (error) {
                  ...
                  {fileHistory.map((file, index) => (
                    <div
                      ...
                      className={`p-4 rounded-lg border ${
                        index === 0 && file.is_current_version
                          ? "border-ub-purple bg-purple-50"
              ...
              <div className="relative border-l-2 border-gray-200 ml-3 space-y-8 pl-6 py-2">
                {approvalHistory.map((step) => (
                  <div key={step.id} className="relative">
                    <span
                      className={`absolute -left-[31px] top-1 h-6 w-6 rounded-full border-2 flex items-center justify-center bg-white
                      ${
