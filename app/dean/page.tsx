...
interface ApprovalWithPrecedingCheck extends ApprovalRequest {
  all_approvals: {
    sequence_order: number;
    status: string;
  }[];
...
export default function DeanDashboard() {
  ...
  useEffect(() => {
    const fetchApprovals = async () => {
      ...
      try {
        // Fetch all dean approvals (assigned to this user OR unassigned)
  ...
  return (
    <div>
      <h1 className="text-3xl font-bold text-gray-900 mb-8">Pending Dean Approvals</h1>
        ...
        <div className="bg-white shadow overflow-hidden sm:rounded-md">
          <ul className="divide-y divide-gray-200">
            {approvals.map((approval) => (
              <li key={approval.id}>
                <Link href={`/dean/assess/${approval.form_submissions.id}?approvalId=${approval.id}`} className="block hover:bg-gray-50">
