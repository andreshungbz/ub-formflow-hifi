  ...
  return (
    <header className="w-full flex flex-col [@media(min-width:960px)]:flex-row [@media(min-width:960px)]:items-center [@media(min-width:960px)]:gap-0 gap-4 px-4 py-3 bg-ub-purple">
      <div className="flex w-full [@media(min-width:960px)]:w-auto items-center justify-between">
        <Link href="/dean" className="text-ub-yellow flex items-center gap-2 shrink-0">
          <Image
            ...
            height={40}
          />
          <span className="text-xl font-bold">UB FormFlow (Dean)</span>
        </Link>
            ...
            >
              {navItems.map((item) => {
                ...
                return (
                  <DropdownMenuItem
                    key={item.href}
                    className={`text-ub-purple font-semiboldhover:text-ub-purple focus:text-ub-purple focus:bg-ub-purple/10 ${
                      active ? 'font-bold' : ''
                    }`}
                    asChild
                  >
