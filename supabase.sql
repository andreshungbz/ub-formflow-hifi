...
CREATE TABLE IF NOT EXISTS form_categories (
  id INT2 GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, -- Changed to INT2 to match user screenshot
  category TEXT UNIQUE NOT NULL,                       -- Changed to TEXT to match screenshot
  created_at TIMESTAMPTZ DEFAULT NOW()
);

ALTER TABLE form_categories ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Public read form_categories"
ON form_categories FOR SELECT
TO public
USING (true);

-- Seed Categories
INSERT INTO form_categories (category) VALUES
('Academic & Enrollment'),
('Student Records'),
('Financial Services'),
('Admissions'),
